...
uint8_t Display::updateTouch(uint16_t *x, uint16_t *y, uint16_t threshold) {
    ...
    if (!this->headless_mode)
        ...
        {
          // The XPT2046 shares the display SPI bus on this panel (LCDWIKI
          // E32R35T). TFT_eSPI's calData/getTouch mapping does not match it, so
          // read the raw ADC and map to pixels directly. Calibrated from the
          // four screen corners (portrait, USB at the bottom).
          uint16_t z = this->tft.getTouchRawZ();
          if (z < threshold)
            return 0;

          uint16_t rawX = 0, rawY = 0;
          this->tft.getTouchRaw(&rawX, &rawY);

          const uint16_t X_RAW_LEFT  = 420;  // screen x = 0
          const uint16_t X_RAW_RIGHT = 3660; // screen x = TFT_WIDTH
          ...
          if (rot == 1) {
            ...
            *y = (uint16_t)constrain(sy, 0, TFT_HEIGHT - 1);
            ...
            if (millis() - lastDbg > 200) {
              lastDbg = millis();
              Serial.print("[TOUCH] z="); Serial.print(z);
              Serial.print(" rawX="); Serial.print(rawX);
              Serial.print(" rawY="); Serial.print(rawY);
              Serial.print(" mappedX="); Serial.print(*x);
              Serial.print(" mappedY="); Serial.println(*y);
            }
          #endif
          return 1;
        }
