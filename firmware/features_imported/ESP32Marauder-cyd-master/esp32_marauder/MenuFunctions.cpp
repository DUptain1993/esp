...
// Pi Terminal add-on (isolated module; safe no-op until its menu node is used).
#include "pi_terminal/pi_terminal_app.h"
...
void MenuFunctions::RunSetup()
{
  ...
  // Pi Terminal: modal CLI page over the encrypted UART link. Runs in place and
  // redraws the main menu on exit so existing UI state is fully restored.
  this->addNodes(&mainMenu, "Pi Terminal", TFTORANGE, NULL, DEVICE, [this]() {
    pi_terminal_open();
