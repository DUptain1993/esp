...
// arduino-esp32 already provides a global `Serial2` bound to UART2. When the GPS
// UART maps to that index (e.g. CYD 3.5"), reuse the core instance to avoid a
// duplicate-symbol link error; otherwise create the requested instance.
#if !defined(GPS_SERIAL_INDEX) || (GPS_SERIAL_INDEX != 2)
