...
// Pi Terminal add-on: encrypted/compressed UART link + on-screen CLI page.
...
void setup()
{
  ...
  // Bring up the Pi Terminal UART link last so it never delays Marauder boot.
  pi_terminal_init();
}
...
void loop()
{
  ...
  // Service the Pi link every loop so streamed output is captured even when
  // the terminal page is closed (non-blocking).
  pi_terminal_pump();
